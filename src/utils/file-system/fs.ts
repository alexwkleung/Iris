...
export async function readFile(dir: string): Promise<unknown> {
    return await invoke('read_file', { dir: dir }).then(
        (v) => v
    ).catch((e) => { throw console.error(e) });
}

export async function readFileRoot(file: string): Promise<unknown> {
    return await readFile(await baseDir("home").then((v) => v) + "/Iris/Notes/" + file).then(
        (v) => v
    ).catch((e) => { throw console.error(e) })
}

export async function readFileFolder(folder: string, file: string): Promise<unknown> {
