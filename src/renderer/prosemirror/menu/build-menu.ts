...
export function buildMenuItems(schema: Schema): MenuItemResult {
    ...
    //eslint-disable-next-line
    if ((mark = schema.marks.strong)) r.toggleStrong = markItem(mark, { title: "Bold", icon: icons.strong });
    //eslint-disable-next-line
    if ((mark = schema.marks.em)) r.toggleEm = markItem(mark, { title: "Italic", icon: icons.em });
    //eslint-disable-next-line
    if ((mark = schema.marks.code)) r.toggleCode = markItem(mark, { title: "Inline code", icon: icons.code });
