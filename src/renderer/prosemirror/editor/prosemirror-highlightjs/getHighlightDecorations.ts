// base from: https://github.com/b-kelly/prosemirror-highlightjs/blob/master/src/getHighlightDecorations.ts
...
export function getHighlightDecorations(
    ...
    blocks.forEach((b) => {
        ...
        //
        // same bug as: https://github.com/b-kelly/prosemirror-highlightjs/issues/21
        //
        // need to comment out the code or else the callback continuously adds to
        // stack and never reaches base case, therefore causing stack overflow
        //
        // leads to app entirely freezing
        //
        ...
        const renderer = new ProseMirrorRenderer(emitter, b.pos, emitter.options.classPrefix);
...
class ProseMirrorRenderer implements Renderer {
    ...
    private expandScopeName(name: string): string {
        if (name.includes(".")) {
            ...
            return [`${this.classPrefix}${prefix}`, ...pieces.map((x, i) => `${x}${"_".repeat(i + 1)}`)].join(" ");
        }
